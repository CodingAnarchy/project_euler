...
time = Benchmark.measure do
  ...
  matrix = grid.map { |line| line.split.map(&:to_i) }

  (0..19).each do |i|
    (0..16).each do |j|
      products << MathUtils.prod(matrix[i][j...j + 3]) # right/left products
      products << MathUtils.prod(matrix[j...j + 3].map { |row| row[i] }) # up/down products
    end
  end
  ...
  (0..16).each do |i|
    (0..16).each do |j|
      products << matrix[i][j] * matrix[i + 1][j + 1] * matrix[i + 2][j + 2] * matrix[i + 3][j + 3] # downwards diagonals
      products << matrix[i + 3][j] * matrix[i + 2][j + 1] * matrix[i + 1][j + 2] * matrix[i][j + 3] # upwards diagonals
    end
  end
end
  
puts "#{products.max} found in #{time.real} seconds"
